...
df = pd.DataFrame({"LogReturn": returns}, index=dates)
df.to_parquet("CL_WTI_1min_processed_test.parquet")
...
def main():
    # 修改为测试路径
    test_dir = "F:/CODE/social/JianZhi/future/WTI_volatility_forecast_replication/04_docs/TestFormula/TSRV"
    output_dir = test_dir + "/output"

    # 使用测试数据
    calculate_daily_tsrv(
        data_path=test_dir + "/CL_WTI_1min_processed_test.parquet",
        freq="1min",
        output_dir=output_dir,
        c=1.0
    )
