...
PATTERNS = {
    "ANN": [
        # BV
        r"CL_WTI_BV_daily_{freq}_ANN_1step_test.parquet",
        r"CL_WTI_BV_daily_{freq}_ANN_cumulative_h{h}_test.parquet",
        # RK
        r"CL_WTI_RK_daily_{freq}_ANN_1step_test.parquet",
        r"CL_WTI_RK_daily_{freq}_ANN_cumulative_h{h}_test.parquet",
    ...
    "HAR": [
        # BV
        r"HAR_BV_{freq}_forecast.parquet",
        r"HAR_BV_{freq}_cumulative_h{h}.parquet",
        # RK
        r"HAR_RK_{freq}_forecast.parquet",
        r"HAR_RK_{freq}_cumulative_h{h}.parquet",
    ...
    "ARFIMA": [
        # BV
        r"ARFIMA_BV_{freq}_forecast.parquet",
        r"ARFIMA_BV_{freq}_cumulative.parquet",
        # RK
        r"ARFIMA_RK_{freq}_forecast.parquet",
        r"ARFIMA_RK_{freq}_cumulative.parquet",
...
def load_forecast(root: str, model: str, asset: str, measure: str, freq: str, h: int) -> Optional[pd.DataFrame]:
    ...
    if model == "ANN":
        # h=1 already σ; h>1 file holds mean(ν²) → sqrt to cumulative σ (RMS)
        if h > 1:
            s = sqrt_clip(s)
