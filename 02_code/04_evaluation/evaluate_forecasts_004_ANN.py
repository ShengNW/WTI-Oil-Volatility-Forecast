...
This version keeps your current, runnable script as the base and adds:
1) **Variance→Volatility (σ) alignment** for both actuals and forecasts,
   with **model-specific logic** (ANN/ARFIMA/GARCH/HAR) per your evidence.
2) **HAR+ANN equal‑weight combination built on already‑aligned σ columns**
   (constructed in evaluation, not read from disk).

Default periods = "auto" (as in your latest). You can still force paper/whole.
"""
...
def load_actual(root: str, asset: str, measure: str, freq: str, h: int) -> Optional[pd.DataFrame]:
    # 现在 test_set 的实际标签已是 σ（标准差），直接读取为 actual。
    if h == 1:
...
def load_forecast(root: str, model: str, asset: str, measure: str, freq: str, h: int) -> Optional[pd.DataFrame]:
    ...
    Model‑specific rules (aligned to paper target ν and your file conventions):
      - ANN: h=1 stored as σ; h>1 stored as mean(ν²) ⇒ take sqrt to get cumulative σ (RMS).
      - ARFIMA: stored as σ; if cumulative column indicates cumVar/variance (i.e., mean ν²), take sqrt.
      - GARCH: h=1 stored as σ; h>1: try rebuild RMS σ from single‑step path; otherwise accept mean-σ fallback (warn).
      - HAR: stored as σ for both h=1 and cumulative (no sqrt here).
      - HAR-ANN: built later (on-the-fly).
    """
    if model == "HAR-ANN":
        return None  # built later from aligned HAR & ANN

    base = os.path.join(root, MODEL_DIR[model])
    # 先尝试 ANN 的 h>1 累积文件（ν²）
    print(f"[B1] ENTER ANN-cum branch | model={model} h={h}")


    chosen = None  # 记录来源路径，便于 DBG
    if model == "ANN" and h > 1:
        cum_dir = os.path.join(root, "03_results", "intermediate_results", "cumulative_forecasts", "ANN")
        print("     cum_dir =", cum_dir)
        tokens = MEASURE_FILE_TOKENS.get(measure, [measure])
        for token in tokens:
            fname = f"{asset}_{token}_daily_{freq}_ANN_cumulative_h{h}_test.parquet"
            path = os.path.join(cum_dir, fname)
            df_try = try_load_parquet(path)
            print("[B1a] TRY:", path, "| exists?", os.path.isfile(path))

            if df_try is not None:
                col = pick_forecast_column(df_try)
                df = df_try[["date", col]].rename(columns={col: model})
                df._source = getattr(df_try, "_source", path)  # 保留来源
                chosen = df._source
                break
        if chosen is not None:
            # ν² -> ν (RMS)
            s = np.sqrt(np.clip(df[model].to_numpy(), 0, None))
            out = df[["date"]].copy()
            out[model] = s
            out._source = chosen
            print("[B4] FINAL RETURN | model=", model, "h=", h,
                  "| used =", getattr(df, "_source", "n/a"),
                  "| col =", (df.columns[1] if df is not None else "n/a"))
            if model == "ANN" and h > 1 and df is not None:
                raw3 = df.iloc[:3, 1].tolist()
                print("     head3 (as read) =", raw3)
                print("     head3 (after sqrt) =", np.sqrt(np.clip(df.iloc[:3, 1].to_numpy(), 0, None))[:3].tolist())

            return out

    # Utility to try explicit patterns first
    ...
    tokens = MEASURE_FILE_TOKENS.get(measure, [measure])
    if model != "ANN" or h == 1:  # ANN 且 h>1 已在上面 return 了
        print("[B3] ENTER patterns/regex fallback | model=", model, "h=", h)
        df = try_patterns(tokens)
        print("[B3] try_patterns result is None?", df is None)
    else:
        df = None  # 理论上不会走到这

    if df is None:
        for token in tokens:
            # 严格：h>1 只搜 cumulative；h=1 才搜 1step
            if h > 1:
                regex = rf".*{token}.*{freq}.*cumulative.*h{h}.*\.parquet$"
            else:
                regex = rf".*{token}.*{freq}.*1step.*\.parquet$"
            df = load_forecast_generic(base, regex, model)
            if df is not None:
                break

    if df is None and model == "GARCH":
        # Also allow base file name without h hint
        df = load_forecast_generic(base, r"garch_11_test\.parquet$", model)

    if df is None:
        return None

    # From here, df has one column named by model. Convert to σ by model rules.
    s = df[model].to_numpy()

    if model == "ANN" and h > 1:
        col_lower = df.columns[1].lower()
        # 1) 如果列名像 “*_cumulative_h5/h10”, “cumvar”, “variance” ——> 明确 ν²，做 sqrt
        if ("cumulative_h" in col_lower) or ("cumvar" in col_lower) or ("variance" in col_lower):
            s = sqrt_clip(s)
        # 2) 否则，如果来源文件路径显式来自 cumulative 目录，也做 sqrt （双保险）
        elif hasattr(df, "_source") and ("intermediate_results" in df._source.replace("\\",
                                                                                      "/") and "/cumulative_forecasts/ANN/" in df._source.replace(
                "\\", "/")):
            s = sqrt_clip(s)
        # 3) 其他情况：默认 **已是 σ**，不 sqrt（同时给出告警）
        else:
            print(
                f"[WARN] ANN h>{h}: column '{df.columns[1]}' from '{getattr(df, '_source', 'n/a')}' looks like sigma already; skip sqrt.")


    elif model == "ARFIMA":
        # h=1 already σ; h>1: if column suggests cumVar/variance → sqrt; else treat as already cumulative σ
        if h > 1:
            col = df.columns[1].lower()
            if ("cumvar" in col) or ("variance" in col) or re.search(r"\bcum.*var\b", col):
                s = sqrt_clip(s)
        ...
        if h == 1:
            ...
            # Try to recompute RMS over future 1..h from single-step file with columns garch_forecast_h1..hN
            ...
            if src is not None:
                ...
                if cols:
                    ...
                    if len(have) == h:
                        ...
                        g = pd.DataFrame({"date": src["date"], model: rms})
                        df = g
                        s = df[model].to_numpy()
                    else:
                        print(f"[WARN] GARCH single‑step missing some horizons for h={h}; fallback to provided cumulative (mean σ). Results not strictly paper‑consistent.")
                else:
                    print("[WARN] GARCH single‑step file lacks horizon columns; fallback to provided cumulative (mean σ).")
            # If we couldn't rebuild, keep s as is (mean σ), still σ‑scale but not paper‑exact

    elif model == "HAR":
        # HAR is stored as σ (including cumulative). Do NOT sqrt here.
        pass

    else:
        # Default: assume σ already
        pass
