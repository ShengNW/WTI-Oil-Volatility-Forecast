...
def load_forecast(root: str, model: str, asset: str, measure: str, freq: str, h: int) -> Optional[pd.DataFrame]:
    ...
    for pat in PATTERNS.get(model, []):
        fname = pat.format(asset=asset, measure=measure, freq=freq, h=h)
        path = os.path.join(base, fname)
        df = try_load_parquet(path)
        if df is not None:
            col = pick_forecast_column(df)
            return df[["date", col]].rename(columns={col: model})
    # Else try soft globs by measure/freq/h
    regex = rf".*{measure}.*{freq}.*(h{h}|1step).*\\.parquet$"
    ...
    for asset in assets:
        ...
        for h in horizons:
            for freq in freqs:
                ...
                rows = []
                per_period_frames: Dict[str, List[pd.DataFrame]] = {k: [] for k in PERIODS}
                ...
                for per_name in PERIODS:
                    ...
                    for rm1, y in actual_aligned.items():
                        # period slice
                        y_per = y[(y.index >= PERIODS[per_name][0]) & (y.index <= PERIODS[per_name][1])]
