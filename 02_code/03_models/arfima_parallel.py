...
class ARFIMA_Model:
    ...
    def train_and_predict_parallel(self, max_workers=None):
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as ex:
            future_to_args = {ex.submit(_worker, args): args for args in task_args}
...
if __name__ == "__main__":
    # 从命令行参数获取根目录和并行度（可选）
    # 用法示例：python arfima_parallel.py "F:/CODE/..." 8
    if len(sys.argv) >= 2:
        root_dir = sys.argv[1]
    else:
