...
def _load_series(root: Path, split: str, measure: str, freq: str) -> pd.Series:
...
def run_for_measure(root_dir: Path, measure: str, freq: str,
    ...
    # Save forecasts
    df_1.to_csv(forecasts_dir / f"CL_WTI_{measure}_daily_{freq}_ANN_1step_test.csv", index_label="DateTime")
    df_h5.to_csv(interm_cumu_dir / f"CL_WTI_{measure}_daily_{freq}_ANN_cumulative_h5_test.csv", index_label="DateTime")
    df_h10.to_csv(interm_cumu_dir / f"CL_WTI_{measure}_daily_{freq}_ANN_cumulative_h10_test.csv", index_label="DateTime")
...
#                    help="Project root directory (WTI_volatility_forecast_replication)")
#     p.add_argument("--measure", type=str, choices=MEASURES, default="MedRV",
#                    help="Realized measure to model")
#     p.add_argument("--freq", type=str, choices=FREQS, default="5min",
#                    help="Intraday sampling frequency associated with daily measures")
...
def main():
    # args = parse_args()
    # root = f"F:/CODE/social/JianZhi/future/WTI_volatility_forecast_replication"#Path(args.root_dir).expanduser().resolve()
    # device = torch.device(args.device)
    # 直接在这里设置所有参数，无需命令行传入
    root_dir = Path("F:/CODE/social/JianZhi/future/WTI_volatility_forecast_replication")  # 你的项目根目录
    measure = "MedRV"  # 可选值: ["RV", "TSRV", "RK", "BV", "MedRV", "JWTSRV"]
    freq = "5min"      # 可选值: ["1min", "5min"]
    ...
    # run_for_measure(root_dir=root, measure=args.measure, freq=args.freq,
