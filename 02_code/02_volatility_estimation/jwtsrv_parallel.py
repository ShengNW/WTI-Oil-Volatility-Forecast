...
#         w_jk = coeffs_j[0][1] if j < J_max else coeffs_j[0][0]
...
#         iv_avg = np.mean(iv_sub) * (N / N_tilde)
...
def process_day(trade_date, intraday_df, wavelet, G):
    ...
    for j in range(1, J_max + 1):
        try:
            ...
            # # 确保我们获取的是系数数组，而不是标量
            # if j < J_max:
            #     w_jk = coeffs_j[0][1]  # 细节系数
            # else:
            #     w_jk = coeffs_j[0][0]  # 近似系数
            # 提取正确系数（trim_approx=True时的索引逻辑）
            if j < J_max:
